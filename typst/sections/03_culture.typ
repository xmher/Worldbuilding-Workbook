...
#mistake-box(
  title: "Mistake #1: The Inconsistent Culture",
  fix: [Establish your rules clearly and enforce them consistently. If characters break rules, show consequences. If rules relax in certain circumstances, establish that pattern early.],
)[
  Cultural rules apply when convenient and disappear when inconvenient. Characters are chaperoned in one scene and alone together the next with no explanation. Stakes that come and go feel fake. If readers can't trust that the rules are real, they won't feel tension about breaking them.
]


#mistake-box(
  title: "Mistake #2: The Paper-Thin Taboo",
  fix: [Make consequences concrete and serious. Show other characters who suffered these consequences so readers know the threat is genuine.],
)[
  Something is supposedly forbidden, but no one seems to actually care. The "forbidden" lovers face disapproval but no real consequences. Forbidden love only matters if the forbidding is real. Stakes need teeth.
]


#mistake-box(
  title: "Mistake #3: The Monolithic Culture",
  fix: [Show variation. Some people enforce rules strictly; others look the other way. Older and younger generations may differ. Your characters should be able to find allies and enemies within their own culture.],
)[
  Everyone in a society thinks the same way, follows the same rules. No internal variation, no rebels, no regional differences.
]
...
#structured-table(
  ...
  rows: (
    ([#strong[Inconsistent Culture:] Rules apply when convenient and disappear when inconvenient], [], []),
...
#hint[Review the culture you've built so far. Check any that apply, then address the ones that need fixing.]
],
)
...
#strong[Reference: Cultural Values & Traits]
...
#workbook-table(
  headers: ("Category", "Possible Values"),
  rows: (
...
#structured-table(
  headers: ("Character", "Relationship to Cultural Values"),
  example-rows: (
...
#open-table(
  headers: ("Routine Element", "Before the Love Interest", "After"),
  example-rows: (
...
#structured-table(
  headers: ("Class", "Home Description"),
  example-rows: (
...
#strong[What's the typical family structure?]
...
#structured-table(
  headers: ("Character", "Family's Stance on the Relationship", "What Power They Have to Interfere"),
  example-rows: (
...
#structured-table(
  headers: ("Meal", "What's Eaten", "How It's Eaten"),
  example-rows: (
...
#open-table(
  headers: ("Setting", "Layout Detail", "How It Affects the Romantic Dynamic"),
  example-rows: (
...
#strong[Reference: Common Superstitions & Omens]
...
#open-table(
  headers: ("The Mistake", "Who Makes It", "How the Other Reacts"),
  example-rows: (
...
#structured-table(
  headers: ("Pattern", "Does It Apply?", "How It Shows Up in Your Story"),
  example-rows: (
...
#open-table(
  headers: ("Activity", "What It Allows (touch, conversation, competition, proximity)", "How Obvious Is It That They're There for Each Other?"),
  example-rows: (
...
#structured-table(
  headers: ("Category", "Taboo Behaviors"),
  example-rows: (
...
#structured-table(
  headers: ("Behaviour", "Consequence for the Protagonist", "Consequence for the Love Interest"),
  example-rows: (
...
#writing-box(
  label: "How does information travel in your society?",
  height: 145pt,
)
...
#structured-table(
  headers: ("Character", "Worst Possible Rumor"),
  example-rows: (
...
#strong[How do your characters protect their reputations while pursuing each other?]
...
#writing-box(
  label: "What excuse or cover story do they use when almost caught?",
  height: 145pt,
)
...
#strong[Is there a moment in your story when they ARE seen?]
...
#structured-table(
  headers: ("Category", "What's Included"),
  example-rows: (
...
#structured-table(
  headers: ("Marker Type", "What Clothing Communicates"),
  example-rows: (
...
#structured-table(
  headers: ("Context", "Wealthy/Noble", "Common/Working"),
  example-rows: (
...
#open-table(
  headers: ("Stage of Relationship", "What's Removed or Revealed", "What It Signifies"),
  example-rows: (
...
#strong[Is there a garment that represents armor or protection that one character removes around the other?]
...
#structured-table(
  headers: ("Violation", "What It Signals"),
  example-rows: (
...
#hint[A character who pauses to close a stranger's eyes or say a prayer reveals something about their capacity for tenderness. A character who walks past without looking reveals something too.]
...
#structured-table(
  headers: ("Character", "How They Were Raised"),
  example-rows: (
...
#strong[How do people typically end up in their professions?]
...
#open-table(
  headers: ("Festival", "When & What It Celebrates", "Key Traditions"),
  example-rows: (
...
#writing-box(
  label: "Which festival provides a romantic opportunity for your characters, and why?",
  height: 145pt,
)
...
- #strong[Political banishment:] The state exerts its power by physically removing the character from the territory through decree, forcing them to survive in an unfamiliar or hostile environment.
- #strong[Self-imposed exile:] A character who voluntarily becomes a pariah to protect others, hiding behind a villain's mask, leaving home to keep a secret safe. The cost is reputation and identity, and the romantic obstacle is that the love interest must see past the performance to the real person underneath.
- #strong[Social ostracism:] The character remains physically present but is treated as invisible, contaminated, or dead. Generational guilt, racial branding, or species-based prejudice mark them as permanent outsiders within their own community.
...
#structured-table(
  headers: ("What They Lose", "Specific Details"),
  example-rows: (
...
#structured-table(
  headers: ("Context", "How They Interact"),
  example-rows: (
...
#structured-table(
  headers: ("Type", "Examples in Your World"),
  example-rows: (
...
#strong[Do your characters come from different cultural backgrounds?]
...
#structured-table(
  headers: ("Misunderstanding", "Description"),
  example-rows: (
...
#strong[Culture Is Behaviour, Not Exposition]
